...
// FBARL ---
#define _FBARL_				// comment this line for regular build
	...
	while (!feof(fp)) 
	{
		...
		if(pch) {
			...
			if(!pszOption) {
				// debug...
#ifdef _FBARL_DEBUG_
				FILE* fpd = fopen(FBARL_DBG_FILE, "w");
				if(fpd) {
...
#ifdef _FBARL_
	iniRead(); // override "g_opt_bUseUNIBIOS" with FB Alpha RL option
#endif
...
static bool init_input(void)
{
   ...
   if(parentrom)	{ fprintf(stderr, "parentrom: %s\n", parentrom); }
   if(boardrom)		{ fprintf(stderr, "boardrom: %s\n", boardrom); }
   if(drvname)		{ fprintf(stderr, "drvname: %s\n", drvname); }
   ...
   /* Afterburner */
   BIND_MAP("Missile", RETRO_DEVICE_ID_JOYPAD_A, 0);
   BIND_MAP("Vulcan", RETRO_DEVICE_ID_JOYPAD_B, 0);
   BIND_MAP("Throttle", RETRO_DEVICE_ID_JOYPAD_Y, 0);
   BIND_MAP("Up", RETRO_DEVICE_ID_JOYPAD_UP, 0);
   BIND_MAP("Down", RETRO_DEVICE_ID_JOYPAD_DOWN, 0);
   BIND_MAP("Left", RETRO_DEVICE_ID_JOYPAD_LEFT, 0);
   BIND_MAP("Right", RETRO_DEVICE_ID_JOYPAD_RIGHT, 0);
   BIND_MAP("Start 2", RETRO_DEVICE_ID_JOYPAD_START, 1);
   BIND_MAP("P1 Low Attack", RETRO_DEVICE_ID_JOYPAD_Y, 0);
   BIND_MAP("P1 High Attack", RETRO_DEVICE_ID_JOYPAD_X, 0);
   BIND_MAP("P1 Weapon", RETRO_DEVICE_ID_JOYPAD_B, 0);
   BIND_MAP("P1 Boost", RETRO_DEVICE_ID_JOYPAD_A, 0);
   BIND_MAP("P2 Low Attack", RETRO_DEVICE_ID_JOYPAD_Y, 1);
   BIND_MAP("P2 High Attack", RETRO_DEVICE_ID_JOYPAD_X, 1);
   BIND_MAP("P2 Weapon", RETRO_DEVICE_ID_JOYPAD_B, 1);
   BIND_MAP("P2 Boost", RETRO_DEVICE_ID_JOYPAD_A, 1);

   /* Bad Dudes */
   BIND_MAP("Fire 4", RETRO_DEVICE_ID_JOYPAD_X, 0);
   BIND_MAP("Fire 5", RETRO_DEVICE_ID_JOYPAD_R, 0);
   BIND_MAP("Up (Cocktail)", RETRO_DEVICE_ID_JOYPAD_UP, 1);
   BIND_MAP("Down (Cocktail)", RETRO_DEVICE_ID_JOYPAD_DOWN, 1);
   BIND_MAP("Left (Cocktail)", RETRO_DEVICE_ID_JOYPAD_LEFT, 1);
   BIND_MAP("Right (Cocktail)", RETRO_DEVICE_ID_JOYPAD_RIGHT, 1);
   BIND_MAP("Fire 1 (Cocktail)", RETRO_DEVICE_ID_JOYPAD_Y, 1);
   BIND_MAP("Fire 2 (Cocktail)", RETRO_DEVICE_ID_JOYPAD_B, 1);
   BIND_MAP("Fire 3 (Cocktail)", RETRO_DEVICE_ID_JOYPAD_A, 1);
   BIND_MAP("Fire 4 (Cocktail)", RETRO_DEVICE_ID_JOYPAD_X, 1);
   BIND_MAP("Fire 5 (Cocktail)", RETRO_DEVICE_ID_JOYPAD_R, 1);

   /* Armed Police Batrider */
   BIND_MAP("P1 Shoot 1", RETRO_DEVICE_ID_JOYPAD_B, 0);
   BIND_MAP("P1 Shoot 2", RETRO_DEVICE_ID_JOYPAD_A, 0);
   BIND_MAP("P1 Shoot 3", RETRO_DEVICE_ID_JOYPAD_X, 0);
   BIND_MAP("P2 Shoot 1", RETRO_DEVICE_ID_JOYPAD_B, 1);
   BIND_MAP("P2 Shoot 2", RETRO_DEVICE_ID_JOYPAD_A, 1);
   BIND_MAP("P2 Shoot 3", RETRO_DEVICE_ID_JOYPAD_X, 1);

   /* Chase HQ */
   BIND_MAP("Turbo", RETRO_DEVICE_ID_JOYPAD_X, 0);

   /* WWF Wrestlefest */
   BIND_MAP("Start 3", RETRO_DEVICE_ID_JOYPAD_START, 2);
   BIND_MAP("Start 4", RETRO_DEVICE_ID_JOYPAD_START, 3);
   BIND_MAP("P1 Turn 1", RETRO_DEVICE_ID_JOYPAD_L, 0);
   BIND_MAP("P2 Turn 1", RETRO_DEVICE_ID_JOYPAD_L, 1);
   BIND_MAP("P1 Turn 2", RETRO_DEVICE_ID_JOYPAD_R, 0);
   BIND_MAP("P2 Turn 2", RETRO_DEVICE_ID_JOYPAD_R, 1);
   BIND_MAP("P1 start", RETRO_DEVICE_ID_JOYPAD_START, 0);
   BIND_MAP("P2 start", RETRO_DEVICE_ID_JOYPAD_START, 1);
   BIND_MAP("Brake 2", RETRO_DEVICE_ID_JOYPAD_L, 0);
   
   BIND_MAP("Brake 3", RETRO_DEVICE_ID_JOYPAD_L2, 0);
   BIND_MAP("Accelerate 2", RETRO_DEVICE_ID_JOYPAD_R, 0);
   BIND_MAP("Accelerate 3", RETRO_DEVICE_ID_JOYPAD_R, 0);
   BIND_MAP("P1 Shot1", RETRO_DEVICE_ID_JOYPAD_B, 0);
   BIND_MAP("P1 Shot2", RETRO_DEVICE_ID_JOYPAD_A, 0);
   BIND_MAP("P2 Shot1", RETRO_DEVICE_ID_JOYPAD_B, 1);
   BIND_MAP("P2 Shot2", RETRO_DEVICE_ID_JOYPAD_A, 1);
   BIND_MAP("P1 Attack Left", RETRO_DEVICE_ID_JOYPAD_Y, 0);
   BIND_MAP("P1 Attack Right", RETRO_DEVICE_ID_JOYPAD_B, 0);
   BIND_MAP("P2 Attack Left", RETRO_DEVICE_ID_JOYPAD_Y, 1);
   BIND_MAP("P2 Attack Right", RETRO_DEVICE_ID_JOYPAD_B, 1);
   BIND_MAP("P1 Button 5", RETRO_DEVICE_ID_JOYPAD_L, 0);
   BIND_MAP("P1 Button 6", RETRO_DEVICE_ID_JOYPAD_R, 0);
   BIND_MAP("P2 Button 5", RETRO_DEVICE_ID_JOYPAD_L, 1);
   BIND_MAP("P2 Button 6", RETRO_DEVICE_ID_JOYPAD_R, 1);
		...
		if (pgi->nInput & GIT_GROUP_MACRO) 
		{
			switch (pgi->nInput) 
			{
				case GIT_MACRO_AUTO:	// Auto-assigned macros
				{
					// Player 1 Macros
					UINT32 port = 0;
					bool state = input_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L);

					if(state && (strcmp(pgi->Macro.szName, "P1 3x Punch") == 0)) 
					{				
						for (INT32 j = 0; j < 4; j++) {
							if (pgi->Macro.pVal[j]) { 
					...
					state = false;
					state = input_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L2);

					if(state && (strcmp(pgi->Macro.szName, "P1 3x Kick") == 0)) 
					{
						for (INT32 j = 0; j < 4; j++) {
							if (pgi->Macro.pVal[j]) { 
								*(pgi->Macro.pVal[j]) = pgi->Macro.nVal[j]; 
					...
					port = 1;
					state = false;
					state = input_cb(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L);

					if(state && (strcmp(pgi->Macro.szName, "P2 3x Punch") == 0)) 
					{				
						for (INT32 j = 0; j < 4; j++) {
							if (pgi->Macro.pVal[j]) { 
								*(pgi->Macro.pVal[j]) = pgi->Macro.nVal[j]; 
					...
					if(state && (strcmp(pgi->Macro.szName, "P2 3x Kick") == 0)) 
					{
						for (INT32 j = 0; j < 4; j++) {
							if (pgi->Macro.pVal[j]) { 
								*(pgi->Macro.pVal[j]) = pgi->Macro.nVal[j]; 
